...
1. **多品牌隔离验证**：使用一次性第二品牌账户登录后，尝试调用或访问 ELYONA 品牌的数据。系统应返回“无权访问其他品牌的数据”或“记录不存在或无权访问”，且不得返回任何业务数据。
2. **验证账户清理**：验证完成后必须立即删除临时账户，避免测试账户长期留存。可在本机 MariaDB 中执行：

```bash
mariadb elyona_fba -e "DELETE FROM users WHERE username = '临时验证用户名';"
```

3. **撤销临时 SSH 公钥**：确认拥有长期可用的 SSH 登录方式后，从 `/root/.ssh/authorized_keys` 中删除本次部署使用的临时公钥条目。
4. **更换 root 密码**：在服务器控制台执行 `passwd root`，设置新的高强度密码；不要通过聊天、邮件或代码仓库传递密码。
5. **清理本地私钥副本**：确认远程公钥已撤销后，安全删除部署环境中的临时私钥文件。
